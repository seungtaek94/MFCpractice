...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LIST_FOR_CHAT, m_ListForChat);
	DDX_Control(pDX, IDC_EDIT_FOR_CHAT, m_EditForChat);
	DDX_Control(pDX, IDC_BTN_SEND, m_BtnSend);
	DDX_Control(pDX, IDC_BTN_CONNECT_SERVER, m_BtnConnect);
}
...
//	ON_BN_CLICKED(IDOK, &CChatClientDlg::OnBnClickedOk)
	...
	ON_BN_CLICKED(IDC_BTN_CONNECT_SERVER, &CChatClientDlg::OnBnClickedBtnConnectServer)
END_MESSAGE_MAP()
...
void CChatClientDlg::OnBnClickedBtnSend()
{
	...
	m_EditForChat.GetWindowText(m_strData);
	m_Client.Send((LPCTSTR)m_strData, m_strData.GetLength() * 2);

	m_strData = _T("");
	m_EditForChat.SetWindowText(m_strData);
	UpdateData(FALSE);
}


void CChatClientDlg::OnBnClickedBtnConnectServer()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CConnectDlg dlg;
	if (dlg.DoModal() == IDOK)
